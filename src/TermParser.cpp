...
{
	index_ += value_.second.size();
	if (static_cast<size_t>(index_) >= view_.size())
	{
		index_ = view_.size();
		return *this;
	}
...
{
	return Iterator(view_.size(), view_);
}
...
size_t binary_size(const TermParser::Iterator & it)
{
	if (!is_binary(it))
	{
	...
	constexpr static auto binary_header_size = sizeof(char) + sizeof(std::uint32_t);
	const auto buff_size = it->second.size();

	if (binary_header_size >= buff_size)
	{
		throw std::runtime_error("Buffer size is too small");
	}

	return buff_size - binary_header_size;
}

void binary(const TermParser::Iterator & it, void * dest, size_t size)
{
	if (!is_binary(it))
	{
		throw parser_error(std::make_error_code(TermParserErrors::TypeError), "binary", it->first);
	...
	auto b_size = binary_size(it);

	if (size < b_size)
	{
		throw std::runtime_error("The passed size is less than the buffer size");
	}

	int idx{0};
	ei_decode_binary(reinterpret_cast<const char *>(it->second.data()), &idx, dest, reinterpret_cast<long *>(&b_size));
}
...
TermParser list(const TermParser::Iterator & it)
{
	...
	// do not include last 106 to list body
	return TermParser(TermParser::BinaryBlock{it->second.data() + idx, it->second.size() - idx - 1});
}
