...
TEST_F(ParserIteratorTest, MapParseAtomNil)
{
	constexpr std::array<const unsigned char, 11> map_term{131, 116, 0, 0, 0, 1, 100, 0, 1, 97, 106};

	TermParser parser(map_term.data(), map_term.size());
	TermParser map_parser = termlib::parse::map(parser.begin());

	auto it = map_parser.begin();
	EXPECT_TRUE(parse::is_atom(it++));
	ASSERT_FALSE(it == map_parser.end());
	EXPECT_TRUE(parse::is_list(it));
}
