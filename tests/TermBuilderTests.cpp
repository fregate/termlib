...
protected:
	...
	TermParser preparing_binary_test(std::span<char> data)
	{
...
TEST_F(TermBuilderTest, BinarySize)
{
	std::array<char, MAXATOMLEN * 3> data{'='};

	auto parser = preparing_binary_test(data);

	const auto it = parser.begin();

	EXPECT_NO_THROW(const auto len = termlib::parse::binary_size(it); EXPECT_EQ(len, data.size()));
}

TEST_F(TermBuilderTest, BinarySizeSmallBuffer)
{
	std::array<char, 1> data;

	builder_.add_binary(data.data(), 1);
	const auto buffer = builder_.buffer();

	TermParser parser(reinterpret_cast<const unsigned char *>(buffer), 1);

	const auto it = parser.begin();

	ASSERT_THROW([[maybe_unused]] const auto len = termlib::parse::binary_size(it), std::runtime_error);
}
...
TEST_F(TermBuilderTest, BinaryNotEnoughSpace)
{
	...
	EXPECT_NO_THROW(const auto len = termlib::parse::binary_size(it); EXPECT_NE(len, recv.size());
					EXPECT_EQ(len, data.size()));
	ASSERT_THROW(termlib::parse::binary(it, recv.data(), recv.size()), std::runtime_error);
}
...
TEST_F(TermBuilderTest, Binary)
{
	...
	EXPECT_NO_THROW(termlib::parse::binary(it, recv.data(), recv.size()); EXPECT_EQ(recv, data));
}
...
TEST_F(TermBuilderTest, BinaryBiggerBuffer)
{
	...
	std::array<char, MAXATOMLEN * 5> recv;
	EXPECT_NO_THROW(termlib::parse::binary(it, recv.data(), recv.size());
